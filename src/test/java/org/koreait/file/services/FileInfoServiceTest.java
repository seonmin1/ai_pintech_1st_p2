...
public class FileInfoServiceTest {
    ...
    void notExistsFileInfoTest() { // 예외가 발생해야 성공하는 테스트
        // 테스트 성공 시 발생된 예외가 반환값
        ...
        assertEquals("NotFound.file", message); // 값 비교
        assertSame(HttpStatus.NOT_FOUND, status); // 객체(주소값) 비교
    }
}
