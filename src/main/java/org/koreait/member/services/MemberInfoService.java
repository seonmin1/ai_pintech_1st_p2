...
public class MemberInfoService implements UserDetailsService {
    ...
    public ListData<Member> getList(MemberSearch search) {
        int page = Math.max(search.getPage(), 1); // 최소 1페이지로 기본값 설정

        int limit = search.getLimit();
        limit = limit < 1 ? 20 : limit; // 목록갯수 최소 20으로 기본값 설정
        ...
        if (dateType.equals("deleteAt")) condition = member.deletedAt; // 탈퇴일 기준
        ...
        if (sDate != null) {
            andBuilder.and(condition.after(sDate.atStartOfDay())); // 시작시간 - 자정 0시 0분 0초
        }

        if (sDate != null) {
            andBuilder.and(condition.before(eDate.atTime(LocalTime.of(23, 59, 59)))); // 종료시간 - 23:59:59
        }
        // 날짜 검색 E
        ...
        List<Member> items = queryFactory.selectFrom(member)
                ...
                .fetchJoin()
                .where(andBuilder)
                .orderBy(member.createdAt.desc()) // 최신 가입 순서대로
                .offset(offset)
                .limit(limit)
                .fetch();
