...
public class BoardUpdateService {
    ...
    public BoardData process(RequestBoard form) {
        ...
        if (mode.equals("edit")) { // 수정 시
            data = boardDataRepository.findById(seq).orElseThrow(BoardDataNotFoundException::new); // 게시글 없으면 예외발생

        } else { // 추가 게시글 등록
            /**
             * 등록될때만 최초 한번 기록되는 데이터 가져오기 (바뀌면 안되는 데이터)
             * - 게시판 설정, 회원
             * - gid
             * - 아이피, UserAgent (브라우저 환경정보)
             */
